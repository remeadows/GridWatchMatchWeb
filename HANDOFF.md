...
**Known residual (kit v0.2.2), deliberately not fixed on this side:** a commit made between the `reconcile()` call and the kit's *decision* — one GET, normally well under a second — is still overwritten when the kit answers an automatic `use_cloud`, because the kit decided on the snapshot it was handed at call time and nothing re-reads the local save in that window. This is the only remaining case where a signed-in player's local change can be replaced without a prompt, and it is bounded by the length of one cloud GET rather than by how long a prompt stays open (which is what the per-slot fix removed). Closing it needs a kit API addition — a `current()` callback the client re-reads at decision time, instead of the one-shot `local` argument — and ships in the next kit release; the application site in `src/App.tsx` carries the same note. Until then the app's own bookkeeping cannot narrow it: the app cannot see the kit's decision point, and pre-emptively holding the answer back would break the "a kit answer is never discarded" invariant that protects the cloud row.
