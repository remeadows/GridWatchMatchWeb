...
- review-20260610T142520Z src/game/BoardScene.ts helper symbols `powerUpEventKeys`/`clearFlashColors`/`powerUpPopStagger`/`unionKeys`: stale; current source defines each helper exactly once near the module end.
...
- review-20260612T140206Z tests/e2e/app.spec.ts board click dispatch override: stale; current source has no `page.mouse.click` references and all board-target clicks call `clickBoardPoint`, which dispatches synchronous PointerEvents to the canvas.
